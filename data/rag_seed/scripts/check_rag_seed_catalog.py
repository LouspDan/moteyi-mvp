#!/usr/bin/env python3
import csv, os, sys, argparse
from pathlib import Path

CSV = Path("data/rag_seed/rag_seed_catalog.csv")
REQUIRED = [
    "id","titre","source_url","langue","grade_level","matiere","type_doc",
    "file_path","checksum","licence","ingested","validated","notes"
]
BASE = Path("data/rag_seed")  # racine des fichiers RAG

def resolve_path(p_raw: str) -> Path | None:
    if not p_raw:
        return None
    p_norm = p_raw.strip().replace("\\", os.sep).replace("//", "/")
    candidates = [Path(p_norm), BASE / p_norm]
    candidates = [Path(os.path.normpath(str(c))) for c in candidates]
    for c in candidates:
        if c.exists():
            return c
    return None

def fail(msg):
    print("❌", msg); sys.exit(1)

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--mode", choices=["strict","ci","lenient"], default=None,
                    help="strict (local): exige les fichiers; ci/lenient: ignore missing_file")
    args = ap.parse_args()

    # Priorité: CLI > ENV > auto (GitHub Actions)
    mode = (args.mode
            or os.getenv("RAG_CHECK_MODE")
            or ("ci" if os.getenv("GITHUB_ACTIONS","").lower() == "true" else "strict"))
    mode = mode.lower()
    print(f"ℹ️ RAG_CHECK_MODE = {mode}")

    if not CSV.exists():
        fail("CSV non trouvé: data/rag_seed/rag_seed_catalog.csv")

    rows = list(csv.DictReader(CSV.open(encoding="utf-8")))
    if not rows:
        fail("CSV vide")

    header = [h.strip() for h in rows[0].keys()]
    missing_cols = [c for c in REQUIRED if c not in header]
    if missing_cols:
        fail(f"Colonnes manquantes: {missing_cols}")

    problems = []
    missing_files = 0

    for r in rows:
        p_raw = (r.get("file_path") or "")
        resolved = resolve_path(p_raw)

        if mode in ("ci","lenient"):
            # En CI: on ne bloque pas sur l’absence de fichiers
            if resolved is None:
                missing_files += 1
            if not (r.get("checksum") or "").strip():
                problems.append(("no_checksum", p_raw))
            continue

        # Mode strict (local)
        if resolved is None:
            problems.append(("missing_file", p_raw))
            continue
        if not (r.get("checksum") or "").strip():
            problems.append(("no_checksum", str(resolved)))

    if mode in ("ci","lenient"):
        if any(k == "no_checksum" for k, _ in problems):
            print("❌ Problèmes (checksums manquants):")
            for k,p in problems:
                if k == "no_checksum":
                    print("-", k, p)
            sys.exit(1)
        print(f"✅ Catalog OK (mode CI). Fichiers absents sur runner: {missing_files}")
        sys.exit(0)

    if problems:
        print("❌ Problèmes détectés:")
        for k,p in problems:
            print("-", k, p)
        sys.exit(1)

    print(f"✅ Catalog OK — {len(rows)} lignes, chemins résolus et checksums présents (mode strict).")

if __name__ == "__main__":
    main()
